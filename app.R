...
server <- function(input, output, session) {
  ...
  output$graficoObservacoes <- renderPlot({
    ...
    ggplot(total_observacoes_df, aes(x = API, y = Observacoes, fill = API)) +
      ...
      theme_classic() +
      labs(
        title = "Total de notificações geocodificadas por API",
        x = "Serviços de API e banco de endereços georreferenciados",
        y = "Notificações"
      ) +
      scale_y_continuous(
  ...
  output$mapaAcuracia <- renderLeaflet({
    ...
    # Adicionar a legenda ao mapa, somente se houver APIs ou Padrão Ouro selecionados
    legend_colors <- unlist(colors[c(selected_apis, if (exibir_cnefe) "Padrão Ouro")])
    legend_labels <- c(selected_apis, if (exibir_cnefe) "Padrão Ouro")
...
onStop(function() {
  ...
  print("Encerrando o aplicativo Shiny.")
})
